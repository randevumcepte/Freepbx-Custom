...
// speechContexts - tüm modellerde ortak kullanılacak
...
// Tek bir recognize çağrısı yap
async function doRecognize(audioContent, modelName, enhanced) {
    const request = {
...
async function transcribeAudio(filePath) {
    ...
    // 1. Önce telephony modeli dene (telefon hattı için optimize)
    try {
        [response] = await doRecognize(audioBytes, "telephony", true);
        console.error("Model: telephony (enhanced)");
    } catch (err1) {
        console.error("telephony modeli başarısız:", err1.message);
        // 2. Fallback: default model (her zaman çalışır)
        try {
            [response] = await doRecognize(audioBytes, "default", false);
            console.error("Model: default (fallback)");
        } catch (err2) {
            console.error("STT tamamen başarısız:", err2.message);
            console.log(JSON.stringify({ success: false, error: err2.message }));
            return;
        }
    }
